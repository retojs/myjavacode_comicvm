/**
 ...
 The PlotParser uses an object oriented* state machine
 to parse the plot text into:

 - charcter names
 - plot items (defined by properties who does or says what how withWhom)

 Each line is either regarded as a

 - character name (a line starting with one of the declared character names)
 - qualifier (a line with text in parentheses)
 - spoken text (a line starting with white-space)
 - description (a line that is none of the above)

 and each line triggers a state transition.

 [* the term "object oriented" should point out
 that the states in this state machine
 inherit from a default state definition
 and can override this definition.
 ]

 */
// TODO finish implementation
// var PlotParser = new Plot_Parser();
...
var PlotParser = {
        ...
        exec: function (input) {
            ...
            $.each(plotContent.split('\n'), function (i, line) {
                ...
                // character declaration or description
                    ...
                    var regexp = new RegExp('(' + matchAnyCharacter + ')');  // matches any character name at the beginning of the line.
                    var matchingCharacters = regexp.exec(line.trim());
                    if (matchingCharacters && matchingCharacters.length > 0) {
                        var who = matchingCharacters[0];
                        var whoWith = [];
                        $.each(PlotParser.getCharacterNames(), function (i, characterName) {
                            if (characterName != who && line.indexOf(characterName) >= 0) {
                                whoWith.push(characterName);
                            }
                        });
    ...
    getCharacterNames: function (reverse) {
        if (!this.parsed.characterNames) {
            ...
            this.parsed.characterNames = characterList;
        }
